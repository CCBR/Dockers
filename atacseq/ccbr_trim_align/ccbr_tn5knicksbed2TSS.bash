...
set -e -x -o pipefail
ARGPARSE_DESCRIPTION="calculate TSS score from tn5knicksbed file"     
source /opt2/argparse.bash || exit 1
argparse "$@" <<EOF || exit 1
parser.add_argument('--tn5knicksbedgz',required=True, help='tn5 knicks bed file gzipped')
...
knicksbed=$(echo $TN5KNICKSBEDGZ|sed "s/.gz//g")
zcat $TN5KNICKSBEDGZ > $knicksbed
...
for f in `ls E*.bed|grep -v tn5nicks`;do echo "bedmap --echo --count $f $knicksbed|awk -F\"\t\" '{if (\$6!=\"+|0\" && \$6!=\"-|0\"){print \$4,\$6}}'|sed \"s/ /|/g\"|awk -F\"|\" -v OFS=\"\t\" '{print \$3,\$5}' > ${f}.counts";done > do_bedmap
head do_bedmap
