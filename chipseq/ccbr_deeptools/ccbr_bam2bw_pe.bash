#!/bin/bash
set -e -x -o pipefail
ARGPARSE_DESCRIPTION="convert treatment & input bams to bigwigs for paired end data"      # this is optional
...
bamCoverage --bam $treatmentsortedbam -o $treatmentbw --binSize 25 --smoothLength 75 --ignoreForNormalization chrX chrY chrM --numberOfProcessors $NCPUS --normalizeUsing RPGC --effectiveGenomeSize $EFFECTIVEGENOMESIZE --centerReads

bamCoverage --bam $inputsortedbam -o $inputbw --binSize 25 --smoothLength 75 --ignoreForNormalization chrX chrY chrM --numberOfProcessors $NCPUS --normalizeUsing RPGC --effectiveGenomeSize $EFFECTIVEGENOMESIZE --centerReads
